#!/system/bin/sh
# @author bomo
# lib_common.sh — OWC 公共函数库
#
# 背景：get_cpu_temp 此前在各守护脚本中各自持有一份拷贝，历史上已发生两次
# 复制漂移事故（单位换算不一致，导致温度判断失效）。参考 Wangshu (Aestas)
# 的"单一实现 + 全链路校验"哲学，收敛到本库统一维护。
#
# 使用方式（守护脚本头部）：
#   . "$MODDIR/lib_common.sh"
# 注意：脚本会被 service.sh 拷贝到 tmp/ 运行，$MODDIR 即脚本所在目录，
#       service.sh / watchdog.sh 必须把本库一并拷贝过去。
#
# 约定：所有温度统一内核标准单位 m°C（0.001°C）；
#       电池温度（power_supply/battery/temp）为 0.1°C 单位，函数内不换算，
#       调用方各自按既有阈值语义使用（保持既有语义不变）。
...
#   - 恒定 95000 是 PMIC 相邻区占位假值（早期实测），真实温度是波动的。
#   - trip 类节点（cpu-hw-trip-*）按 type 排除，与触发阈值解耦。
get_cpu_temp() {
    local max=0 t type
