...
#
# @author bomo v1.4.9（2026-09-16 实测后拍板）：**熔断后零处置**语义边界声明（纯注释，无逻辑变更）。
#   已把"为什么不再加二级处置"的完整论证写进下方参数段后的专章，防日后反复推翻。
#   一句话：熔断的唯一使命是解除"充电供热"，解除后模块**无可用杠杆**（充电已落到 2.4~3.1W，
#   温度 100% 来自游戏负载），再动即为负收益。详见参数段后「熔断后零处置」章。
#
...
# ====================================================
# @author bomo v1.4.9（2026-09-16 实测后拍板）：**熔断后零处置**（语义边界声明）。
#
#   ★ 这是本模块的设计边界，**不是待办、不是缺陷、不要"修"**。
#     任何形式的"熔断后继续压温"（二次降流 / 强制冷却期 / 拉长熔断时间 /
#     降低复归线"多撑一会儿") 都属越界，见下论证。
#
#   实测证据（异环 + 亮屏快充，2026-09-16 00:30~00:32，28 点 × 3s）：
#     CPU  96.1~98.5°C   —— 全程在熔断线 92°C 之上 4~6.5°C，**既不升也不降**
#     GPU  92.3~95.0°C   —— 同上
#     电池 42.4~42.5°C   —— 稳定未越线
#     cool_down = 5      —— 恒定，系统完整接管
#     电池侧功率 4084mV × 594~753mA ≈ **2.4~3.1W**（几乎不充电）
#   ⇒ 熔断（交还 cool_down + 恢复 ORMS/horae）**已完全达成其唯一使命**：
#     解除了"充电供热"这一路。此后温度 100% 来自游戏自身负载，
#     且充电电流已落到 2.4~3.1W 的噪声级别 —— 模块手里**已无可用杠杆**。
#
#   ⇒ 为何"再进一步"是负收益（三条各自独立成立）：
#     ① 无执行器：模块只能开关快充，管不了 SoC 频率/电压。真要压温唯一手段是
#        "持续不恢复快充"，那等于把自己降级成调速器 —— 与 v1.4.7 用户拍板的
#        "保险丝语义"直接冲突（用户原话："一到阈值立即熔断并完全交还系统"）。
#     ② 与系统热管理打架：CPU 稳定贴在 96~98.5°C 是系统选定的**工作点**
#        (operating point) —— 系统有意让它稳在那儿，PMIC 95°C 硬复位线并未触发，
#        说明系统认为当前状态在许可范围内。模块再插一手会让两套温控互相踩，
#        结果不可预测（可能是更差的调度抖动，而非降温）。
#     ③ 数据不支持：电流已 <753mA，充电供热可忽略。此时"少充点电"换不来降温，
#        只会白白牺牲充电体验 —— 纯负收益。
#   ⇒ 用户 2026-09-16 原话确认：「系统能处理好的吧，我做过多的干涉也许可能还有负面效果？」
#     —— 判定正确，采纳。
#
#   真正需要模块介入的场景仍然正常生效：低负载游戏 / 有散热器 / 不玩游戏时，
#   温度不到阈值 ⇒ 完全不干涉，快充照跑（这正是保险丝语义的价值）。
# ====================================================
